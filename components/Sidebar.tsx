...
export default function Sidebar({ role }: { role: "admin" | "user" }) {
  ...
  const activeColor =
    role === "admin" ? "bg-purple-600 text-white" : "bg-blue-500 text-white";
  ...
  return (
    <aside className={`w-60 h-screen ${bgColor} text-black flex flex-col`}>
      <div>
        <div className="p-6 mt-3 mb-3 text-l font-bold">BISMON</div>
        <nav className="px-4">
          <ul className="space-y-2">
            {menuItems.map((item) => (
              <li key={item.href}>
                <Link
                  href={item.href}
                  className={`text-sm mb-4 flex items-center gap-3 block py-2 px-3 rounded-[2vw] transition-colors ${
                    pathname === item.href
                      ? activeColor
      ...
      {role === "admin" && (
        <div className="p-4 ">
          <button className="w-full cursor-pointer font-semibold text-white text-sm bg-gray-500 hover:bg-purple-600 py-2 rounded-[1vw]">
            Logout
          </button>
        </div>
      )}
