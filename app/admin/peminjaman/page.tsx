...
export default function DataPeminjamanAdminPage() {
  ...
  const [statusPeminjaman, setStatus] = useState("all");
    
  const filteredData = dataPeminjaman.filter((item) => {
    const matchSearch = item.namaPeminjam.toLowerCase().includes(search.toLowerCase());
    const matchStatus = statusPeminjaman === "all" || item.statusPeminjaman === statusPeminjaman;
  ...
  <div className="flex flex-wrap items-center gap-1">
    ...
    <Select onValueChange={setStatus} defaultValue="all">
      ...
      <SelectContent className="text-xs">
        ...
        <SelectItem value="Laptop" className="text-[10px]">Aktif</SelectItem>
        <SelectItem value="TV" className="text-[10px]">Selesai</SelectItem>
      </SelectContent>
    </Select>
          ...
          <table className="w-full text-xs border-collapse">
            <thead className="bg-blue-100 text-left sticky top-0 z-10">
              <tr>
                ...
                <th className="border p-2">Tanggal/Deadline Pengembalian</th>
                <th className="border p-2">Tujuan</th>
                <th className="border p-2">Keterangan</th>
                <th className="border p-2">Status Peminjaman</th>
            ...
            <tbody>
              {filteredData.map((item, index) => (
                <tr key={item.idPeminjaman} className="hover:bg-gray-50">
                  ...
                  <td className="border p-2">{item.tanggalKembali}</td>
                  <td className="border p-2">{item.tujuan}</td>
